...
# Add event filter
oc new-build https://github.com/alexgroom/cnw3.git --context-dir=event-display-go --name=event-filter --strategy=docker
kn service create event-filter --image=image-registry.openshift-image-registry.svc:5000/$SERVERLESS_PROJECT/event-filter --scale 1 \
--label 'bindings.knative.dev/include=true' -e SEARCH="Created pod:" -e ITEM="web-"
kn trigger create events-trigger6 --sink ksvc:event-filter --filter type=dev.knative.apiserver.resource.add
# Add sink binding
# Add the K_SINK sink binding for the filter to broker, or failing that hard code the broker value
kn source binding create bind-api --subject Service:serving.knative.dev/v1:event-filter --sink broker:default
#
